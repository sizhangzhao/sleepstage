...
class CNNRNNPostProcessor:

    """
    Post processor for CNN RNN model
    ...
    def __call__(self, batches: [List[Tuple]]):
        ...
        for batch in batches:
            label = batch[1] - 1
            feature = map(lambda epoch: np.expand_dims(epoch.to_numpy().transpose(), axis=0), batch[0])
            feature = reduce(lambda x1, x2: np.concatenate((x1, x2), axis=0), feature)
            length = len(label)
            lengths.append(length)
            padded_label = np.zeros((1, self.max_length))
            padded_label[0, 0:length] = label
            padded_feature = np.zeros((1, self.max_length, feature.shape[1], feature.shape[2]))
